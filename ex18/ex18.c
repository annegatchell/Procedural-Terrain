 ...
 *
 *  'm' Shadow map/Image(mode)
 *  'o' Objects
 *  +/- Light elevation
 *  []  Light position
 *  's' Start/stop light animation
 *  <>  Number of slices in objects
 *  'a' Toggle axes
 *  'b' Toggle box
...
int          shadowdim; // Shadow texture width&height
...
void InitMap()
{
   ...
   glGetIntegerv(GL_MAX_TEXTURE_UNITS,&n);
   if (n<2) Fatal("Too few texture units %d\n",n);
